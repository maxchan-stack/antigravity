 ...
 * Project: Student Grade Inquiry System V9.0 Campus Edition
...
const CONFIG = {
    ...
    // Admin Alert
    ADMIN_EMAIL: 'REPLACE_WITH_ADMIN_EMAIL', // <--- 請修改此處 (Enter Admin Email)
...
function findStudentData(studentId) {
    ...
    headers.forEach((header, colIndex) => {
        ...
        if (['學號', '姓名', '查詢碼', 'Email', '班級', '座號', '備註', '缺交'].includes(header)) return;
    ...
    ss.getSheets().forEach(s => {
        ...
        for (let i = 1; i < d.length; i++) {
            ...
            if (em && String(em).includes('@') && row[cIdx]) {
                try {
                    // 使用手動輸入的正確網址
                    const subject = '【重要】校園段考成績查詢碼 & 使用說明';
